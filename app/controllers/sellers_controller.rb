  ...
  def index
    ...
    respond_to do |format|
      ...
      format.json { 
        ...
        require 'net/http'
        response = Net::HTTP.get_response("https://api.instagram.com/v1/users/self?access_token=",params[:access_token].to_s)

        case response
          when Net::HTTPSuccess then 
            parsed_response = JSON.parse(response.body)
            @seller = Seller.new(:name => parsed_response["data"]["username"],:description => parsed_response["data"]["bio"], :instagram_id => parsed_response["data"]["id"], :priority => 1, :access_token => params[:access_token]) 
            if @seller.save
